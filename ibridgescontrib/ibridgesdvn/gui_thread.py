...
class TransferDataThread(PySide6.QtCore.QThread):
    ...
    def __init__(self, ienv_path: Path, logger, dvn_ops: DvnOperations,
                 dvn_url: str, dvn_token: str, 
                 tempdir: Path, dataset_id: str, checksum: bool):
        """Pass parameters.
        ...
        self.irods_paths = [IrodsPath(self.thread_session, ip)
        ...
        for irods_path in self.irods_paths:
            if irods_path.dataobject_exists():
                try:
                    ...
                    self.logger.info("DATAVERSE: Upload %s --> %s", str(local_path), self.dataset_id)
                    # check checksums
                    if self.checksum:
            ...
            print(file_count, len(self.irods_paths)), file_failed
            self.current_progress.emit(
                [file_count, len(self.irods_paths), file_failed]
                )
